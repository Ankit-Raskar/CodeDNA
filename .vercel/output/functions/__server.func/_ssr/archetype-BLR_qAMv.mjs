import { t as getServerFnById } from "../__23tanstack-start-server-fn-resolver-D7Ed-Ao5.mjs";
import { i as TSS_SERVER_FUNCTION, l as createServerFn } from "./esm-Dova13aH.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/archetype-BLR_qAMv.js
...
var fetchGithubProfile = createServerFn({ method: "GET" }).inputValidator((d) => {
	...
	if (d?.token) {
		...
		if (t.length > 0) {
			if (t.length <= 255 && /^(gh[pousr]_[A-Za-z0-9_]+|github_pat_[A-Za-z0-9_]+)$/.test(t)) token = t;
		}
	}
	return {
		username: u,
...
function computeAchievements(data) {
	...
	const recentPushes = data.repos.filter((r) => Date.now() - +new Date(r.pushed_at) < 1e3 * 60 * 60 * 24 * 60).length;
	return [
		{
			id: "react",
			name: "React Master",
			description: "Built projects with React.",
			earned: isReactMaster,
			rarity: "Rare"
		},
		{
			id: "opensource",
			name: "Open Source Hero",
			description: "Earned over 100 stars on your repositories.",
		...
		},
		{
			id: "ai",
			name: "AI Builder",
			description: "Built AI-related projects (LLM, Python, etc).",
			earned: data.totals.topics.some((t) => /ai|ml|llm|gpt|openai|python/i.test(t)),
			rarity: "Rare"
		},
		{
			id: "night",
			name: "Night Owl",
			description: "Pushed code consistently during late hours (active recently).",
			earned: recentPushes >= 10,
			rarity: "Common"
		},
		{
			id: "hackathon",
			name: "Hackathon Warrior",
			description: "Created many repositories rapidly.",
			earned: data.repos.length >= 30,
			rarity: "Epic"
		},
		{
			id: "addict",
			name: "Code Addict",
			description: "Created more than 50 repositories.",
		...
		},
		{
			...
			description: "Used 5 or more different programming languages.",
			earned: langs.length >= 5,
			rarity: "Rare"
		},
		{
			id: "bug",
			name: "Bug Slayer",
			description: "Fixed bugs and maintained old projects.",
		...
		},
		{
			...
			description: "Reached over 100 repositories or 1000 stars.",
			earned: data.totals.repos >= 100 || data.totals.stars >= 1e3,
			rarity: "Legendary"
		},
		{
			id: "consistent",
			name: "Consistent Coder",
			description: "Has been coding on GitHub for 3+ years.",
