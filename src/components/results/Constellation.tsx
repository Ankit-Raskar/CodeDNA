      ...
      <div className="relative h-[420px] w-full md:h-[520px] overflow-hidden rounded-2xl">
        ...
        >
          {({ zoomIn, zoomOut, resetTransform }) => (
            <>
              {/* Zoom Controls */}
              <div className="absolute right-3 top-3 z-20 flex flex-col gap-1.5 rounded-xl border border-white/10 bg-white/5 p-1.5 backdrop-blur-md">
                <button
                  onClick={() => zoomIn()}
      ...
      <div className="relative mt-4 flex flex-wrap gap-2 px-1 text-xs">
        <button
          ...
          className={`rounded-full border px-3 py-1 transition ${
            !activeLang ? "border-white/20 bg-white/15 text-white" : "border-white/10 bg-white/5 text-white/60 hover:bg-white/10"
          }`}
        >
          All
        </button>
